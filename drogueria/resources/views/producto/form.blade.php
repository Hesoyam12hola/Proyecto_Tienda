    ...
    <div class="col-md-12">
        ...
        <div class="form-group mb-2 mb20">
            ...
            <input type="text" name="cantidad" class="form-control @error('cantidad') is-invalid @enderror" 
                   value="{{ old('cantidad', $producto?->cantidad) }}" id="cantidad" placeholder="Cantidad">
        ...
        {{-- CAMPO MODIFICADO: SELECT PARA TIPO PRODUCTO --}}
        <div class="form-group mb-2 mb20">
            <label for="tipoproducto_id" class="form-label">Tipo de Producto</label>

            <select name="tipoproducto_id" id="tipoproducto_id"
                    class="form-control @error('tipoproducto_id') is-invalid @enderror">

                <option value="">Seleccione un tipo...</option>

                @foreach ($tipos as $tipo)
                    <option value="{{ $tipo->id }}"
                        {{ old('tipoproducto_id', $producto->tipoproducto_id ?? '') == $tipo->id ? 'selected' : '' }}>
                        {{ $tipo->id }} - {{ $tipo->nombre }}
                    </option>
                @endforeach
            ...
            {!! $errors->first('tipoproducto_id', '<div class="invalid-feedback" role="alert"><strong>:message</strong></div>') !!}
        </div>
